...
function createRandomizedReview(gender, name, assessment, sampleSentences)
{
	...
	if (assessment)
	{
		review.text = "Assessment: "
	}
	else
	{
		review.text = ""
	}
	review.text += IntroductorySentenceBank[Math.floor(Math.random() * IntroductorySentenceBank.length)].replace("name", name) + ". "
	if (assessment)
	{
		review.text += AssessmentSentenceBank[Math.floor(Math.random() * AssessmentSentenceBank.length)].replace("name", name) + ". "
	}
	while (review.text.split(' ').length < 60)
	{
		var randomIndex = Math.floor(Math.random() * sampleSentences.length);
		if (!usedIndexes.includes(randomIndex))
		{
			...
			if (Math.random() > 0.75)
			{
				randomSentence = randomSentence.replace(/He/g, name);
			}
			if (i % 2 == 0)
			{
	...
	review.text += ConclusionSentenceBank[Math.floor(Math.random() * ConclusionSentenceBank.length)] + "."
	if (review.text.indexOf(name) == -1)
	{
		review.text = review.text.replace("He", name)
	}
	if (gender == 'She')
	{
		review.text = review.text.replace(/He/g, "She")
	}
	viewReview(review)
	return review;
}
...
{
	var premadesDiv = document.getElementById('premades')
	var reviewString = "<div class = 'review'><textarea rows='7' cols='90'>" + review.text + "</textarea></div>"
	premadesDiv.innerHTML += reviewString
}
...
function generate()
{
	...
	{
		case '1':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, NonReaderSentenceBank);
			break;
		case '2':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, NonReaderSentenceBank);
			break;
		case '3':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, ReaderSentenceBank);
			break;
		case '4':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, ReaderSentenceBank);
			break;
		case '5':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, UpperLevelSentenceBank);
			break;
		case '6':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, UpperLevelSentenceBank);
			break;
		case '10':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, PreVipSentenceBank);
			break;
		/*
		case '11':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUAPreVIP);
			break;
		case '12':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUA1);
			break;
		case '13':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUA2);
			break;
		case '14':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUA3);
			break;
		case '15':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUA4);
			break;
		case '16':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUA5);
			break;
		case '17':
			createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, contentLevelUA6);
			break;
		*/
		default:
			//createRandomizedReview(form.gender.value, form.name.value, form.assessment.checked, endingSentences);
			break;
	}
